...
class Depan extends Component {
  constructor(props) {
    ...
    this.url = "http://192.168.103.223/api/api.php?op=";
  }
  componentDidMount(){
      this.ambilListData()
  }
      ...
      if(this.state.nama == '' || this.state.alamat == ''){
        alert('Silakan masukkan nama dan alamat');
          ...
          if(this.state.idEdit){
            var urlAksi = this.url+"update&id="+this.state.idEdit;
          }else{
            var urlAksi = this.url+"create";
          }
  ...
  async klikEdit(id){
    await fetch(this.url+"detail&id="+id)
    .then((response)=>response.json())
    .then((json)=>{
  ...
  async klikDelete(id){
    await fetch(this.url+"delete&id="+id)
    .then((response)=>response.json())
    .then((json)=>{
